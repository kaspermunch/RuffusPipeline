...
class InvalidIdentifier(XgridInterfaceError):
...
class InvalidAction(XgridInterfaceError):
...
#             return {}
#     else:
#         raise XgridInterfaceError("xgrid command error: %s" % result[0])
        ...
        if result[1]:
            ...
            def tagify(token):
                if re.match(r'\d+', token):
                    return '<integer>%s</integer>' % token
                elif re.match(r'\d+\.\d+', token):
                    return '<real>%s</real>' % token
                elif re.match(r'".*"', token) or re.match(r"'.*'", token):
                    return '<string>%s</string>' % token
                else:
                    return '<string>%s</string>' % token
                    
            regex = re.compile(r'\s+((\S+)\s+=\s+(.*?)\s*;)')
            for m in regex.findall(plist):
                plist = plist.replace(m[0], '<key>%s</key> %s' % (m[1], tagify(m[2])))

            plist = re.sub(r'(\S+)\s+=', r'<key>\1</key>', plist)
            plist = re.sub(r"\s(\d+),*\s", r'<integer>\1</integer>', plist)
            plist = re.sub(r"\s(\d+\.\d+),*\s", r'<real>\1</real>', plist)
            plist = plist.replace('(', '<array>').replace(')', '</array>')
            plist = plist.replace('{', '<dict>').replace('}', '</dict>')
            plist = plist.replace(';', '')

            try:
                return plistlib.readPlistFromString(plist)
            except:
                raise XgridInterfaceError("Error parsing old style plist: \n%s" % result[1])
        else:
            return {}
    else:
        raise XgridInterfaceError("xgrid command error: %s" % result[0])
    ...
    def __init__(self, hostname=0, password=0, kerberos=False):
        ...
        if hostname == 0:
            if defaultXgridHostname:
                ...
                raise XgridInterfaceError('No controller hostname specified')
        else:
...
class JobManager:
    ...
    def printJobs(self):
        ...
        self._updateJobs()
        print "##########################################################"
        print  "%-8s %-28s %-12s %-12s" % \
            ("id", "Date Started", "Status", "CPU")
        print "##########################################################"
        for j in self._jobs:
            j.printInfo(0)
...
class GridManager:
    ...
    def _updateGrids(self):
        ...
        self._gridIDs = [processID(x) for x in result['gridList']]
...
class Controller(JobManager, GridManager):
    ...
    def submit(self, cmd, args='', stdin='', indir='', email='', art='', gridID=u'0', silent=False):
        ...
        id = j.submit(cmd, args, stdin, indir, email, art, gridID, silent)
        return j
...
class Grid(JobManager):
    ...
    def submit(self, cmd, args='', stdin='', indir='', email='', art='', silent=False):
        ...
        j = Job(connection=self._connection)
        id = j.submit(cmd, args, stdin, indir, email, art, self.gridID, silent)
        return j
...
#        submitString = stdinString + indirString + emailString + \
        submitString = stdinString + indirString + emailString + artString + \
            cmd + ' ' + argString
    ...
    def stop(self, silent=False):
        """Stops the job."""
        self.perform('stop', silent=silent)
        
    def suspend(self, silent=False):
        """Suspends the job."""
        self.perform('suspend', silent=silent)
        
    def resume(self, silent=False):
        """Resumes the job."""
        self.perform('resume', silent=silent)
        
    def delete(self, silent=False):
        """Deletes the job."""
        self.perform('delete', silent=silent)

    def restart(self, silent=False):
        """Restarts the job."""
        self.perform('restart', silent=silent)
    ...
    def printInfo(self,verbose=True):
        ...
        self._updateInfo()
        if verbose == False:
            output = "%-8s %-28s %-12s %-12s" % \
                (self.jobID, self.getInfo('dateStarted'),
                 self.getInfo('jobStatus'),
                 self.getInfo('activeCPUPower'))
            print output
        elif verbose == True:
            print "{"
...
if __name__ == "__main__":
    ...
    conn = Connection(hostname='10.11.101.253',password='birc')
    print conn
    cont = Controller(conn)
    print cont.gridIDs()
    print cont.grids()
...
#     #j.printSpecification()
# 
    j = g.submit(cmd='touch munch.txt', indir="kasperindir")
#    j = g.submit(cmd='/usr/bin/cal', indir=indir)
#    j = g.submit(cmd='/usr/bin/cal -j')
# #    j = g.submit(cmd='asfasdfasl')
    j.results(block=10, stdout="job229.out",stderr="job229.err", outdir="kasperdir")
    j.printInfo(verbose=False)

# FIXME make a full graph of all module dependencies, not just the top level...
