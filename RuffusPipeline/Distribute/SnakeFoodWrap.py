...
def gendeps(pythonFile):
#     import optparse
#     parser = optparse.OptionParser(__doc__.strip())

    opts_internal = 0
    opts_external = False
    opts_ignores = def_ignores + ['SnakeFoodWrap.py']
    opts_verbose = 0
    opts_quiet = 1
    opts_follow = True
    opts_print_roots = False
    opts_do_pragmas = True
    opts_ignore_unused = False
...
#     opts, args = parser.parse_args()
    opts_verbose -= opts_quiet
    setup_logging(opts_verbose)
# 
#     if not args:
#         logging.warning("Searching for files from current directory.")
#         args = ['.']

    args = [pythonFile]#, __file__] # __file__ is included because snakefood seems to have trouble finding itself...

    assert not (opts_internal and opts_external)
#     if opts_internal and opts_external:
#         parser.error("Using --internal and --external at the same time does not make sense.")

    if opts_print_roots:
        inroots = find_roots(args, opts_ignores)
    ...
    for arg in args:
        fn = realpath(arg)
        assert exists(fn), "Filename '%s' does not exist." % fn
#         if not exists(fn):
#             parser.error("Filename '%s' does not exist." % fn)
    ...
    inroots = find_roots(args, opts_ignores)
    if (opts_internal or opts_external) and not inroots:
    ...
    fiter = iter_pyfiles(args, opts_ignores, False)
    while 1:
        newfiles = set()
        for fn in fiter:
            ...
            if is_python(fn):
                files, errors = find_dependencies(
                    fn, opts_verbose, opts_do_pragmas, opts_ignore_unused)
            ...
            # no dependency.
            from_ = relfile(fn, opts_ignores)
            if from_ is None:
                continue
            infrom = from_[0] in inroots
            if opts_internal and not infrom:
                continue
            if not opts_external:
            ...
            for dfn in files:
                ...
                to_ = relfile(xfn, opts_ignores)
                into = to_[0] in inroots
                if (opts_internal and not into) or (opts_external and into):
                    continue
                allfiles[from_].add(to_)
                newfiles.add(dfn)

        if not (opts_follow and newfiles):
            break
        else:
            fiter = iter(sorted(n for n in newfiles if not n.startswith('/Library/Frameworks/Python')))
    ...
    modules = set()
    for (from_root, from_), targets in sorted(allfiles.iteritems(), key=itemgetter(0)):
        for to_root, to_ in sorted(targets):
            modules.add((to_root, to_))
    return list(modules)
